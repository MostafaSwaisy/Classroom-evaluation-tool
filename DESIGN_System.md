...
| Bioluminescent Teal | `#34e8bb` | `--color-bioluminescent-teal` | Primary action — filled CTA buttons (Download), in-text links, nav accent, active states. The single chromatic color permitted to carry interactive meaning; glows against graphite ink |
...
Transparent or near-transparent background with #ffffff text and a subtle white-on-graphite border, 9999px radius, 12px 24px padding, DM Sans weight 500. Sits beside the teal Download button as a lower-commitment option.
...
Transparent or matching graphite ink background. Logo left, center menu with dropdown indicators, user icon in a thin circle outline (32px), and a teal Download pill button right. 12px DM Sans menu items, 6px gap between items.
...
Horizontal row of grayscale corporate logos (Cisco, Mercado Libre, Dell, VMware, etc.), evenly spaced, white or light gray on graphite ink. Logos sit at uniform height (~28-32px) with consistent vertical alignment. No card backgrounds or dividers between them.
...
Rounded rectangle (6px) in a slightly lighter graphite than the canvas, containing product imagery. Functions as a 'window' into the IDE. Should feel embedded in the page, not floating.

## Do's and Don'ts

### Do
- Set every page and section background to #111417 (Graphite Ink) — the dark canvas is the brand identity
...
Graphite tonal ramp as used across the generated screens (M3-style container steps):

| Level | Name | Value | Token | Purpose |
|-------|------|-------|-------|---------|
| -1 | Container Lowest | `#0c0e12` | `--surface-container-lowest` | Deepest wells — inset code panes, sunken list areas |
| 0 | Graphite Canvas | `#111417` | `--surface` / `--background` | Full-bleed page background — every section sits on this graphite |
| 1 | Container Low | `#191c1f` | `--surface-container-low` | Sidebar column, docked utility panels |
| 2 | Container | `#1d2023` | `--surface-container` | Standard card / panel surface |
| 3 | Container High | `#282a2e` | `--surface-container-high` | Active panel header, popover, raised row |
| 4 | Container Highest | `#333539` | `--surface-container-highest` | Floating dialog, inspector module, hover fills |
| 5 | Surface Bright | `#37393d` | `--surface-bright` | Brightest tonal step — selected/active chrome |

Structural lines: `--outline-variant #3b4a44` (hairline dividers, table grid), `--outline #84948d` (stronger separators, disabled glyphs). Text on graphite: `--on-surface #e1e2e7` (body), `#ffffff` (emphasis/titles), `--on-surface-variant #bacac2` (secondary).

Accent / role colors (unchanged from the screens): `--primary #93ffdb`, `--primary-container #34e8bb` (Bioluminescent Teal — the single action accent), `--on-primary-container #00644e`, `--secondary #dcb8ff` (Lavender), `--tertiary-container #ffb9ec` (Bubblegum), `--error #ffb4ab`.
...
- background: #111417 (Graphite Ink)
...
**5. Client Logo Strip**
Horizontal row of 8-10 grayscale client logos (32px height) on #111417 background. Logos rendered in #ffffff or #c0c1c6 (no color), evenly distributed with flexbox justify-content: space-between, max-width 1280px, vertically centered, 40px vertical padding above and below. No card backgrounds, no dividers between logos.
...
- **Linear** — Same graphite dark canvas with a single vivid accent for primary actions, and the same whisper-thin large display headlines that use size over weight for authority
...
:root {
  ...
  /* Surfaces — graphite tonal ramp (as used in the generated screens) */
  --surface-container-lowest: #0c0e12;
  --surface-graphite-canvas: #111417; /* --surface / --background */
  --surface-container-low: #191c1f;
  --surface-container: #1d2023;
  --surface-container-high: #282a2e;
  --surface-container-highest: #333539;
  --surface-bright: #37393d;
  --surface-lifted-graphite: #1d2023;
